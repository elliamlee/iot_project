...
GPIO.setup(19, GPIO.OUT) #RED LED
GPIO.setup(16, GPIO.OUT) #BLUE LED

def send_metrics(h,t, time):
...
class Device(object):
  ...
  def update_sensor_data(self):
    ...
    """
    h, t = dht.read_retry(dht.DHT11, 21)
    print ("Temp={0:0.1f}*C   Humidity={1:0.1f}%".format(t, h))
    self.temperature = t;
    self.hum = h;

    send_metrics(h,t, time.time())
  ...
  def on_message(self, unused_client, unused_userdata, message):
    ...
    self.redled = data['RedLED']
    self.blueled = data['BlueLED']

    if self.redled == True:
      print 'redled turned on.'
      GPIO.output(19, GPIO.HIGH)
    elif self.redled == False:
      print 'redled turned off.'
      GPIO.output(19, GPIO.LOW)

    if self.blueled == True:
      print 'blueled turned on.'
      GPIO.output(16, GPIO.HIGH)
    elif self.blueled == False:
      print 'blueled turned off.'
      GPIO.output(16, GPIO.LOW)
      

    #sseung if data['fan_on'] != self.fan_on:
      # If we're changing the state of the fan, print a message and update our
      # internal state.
      #sseung self.fan_on = data['fan_on']
      #sseung if self.fan_on:
      #sseung  print 'Fan turned on.'
      #sseungelse:
      #sseung  print 'Fan turned off.'
...
def main():
  ...
  for _ in range(args.num_messages):
    ...
    # string.
    payload = json.dumps({'temperature': device.temperature,'humidity': device.hum})
